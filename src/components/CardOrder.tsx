...
const CardOrder = (props: any) => {
  ...
  const [source, setSource] = React.useState("Pillikan");
  ...
  const getOtp = () => {
    ...
    ReactGA.event({
      category: "Forma_zayavki_OTP_1_B",
      action: "Success_zayavki_OTP_1_B",
    });
    setLoading(true);
    setTimer(90);
    api.authOtp
  ...
  const onSubmitOtp = () => {
    setLoading(true);
    ReactGA.event({
      category: "Success_OTP_2_kartakarta_B",
      action: "Success_OTP_2_B",
    });
    api.authOtp
      .confirmOtp({
        iin: iin,
